...
class FinetuneRGBD(pl.LightningDataModule):
    def __init__(
        self,
        train_dir: str = None,
        val_dir: str = None,
    ...
    def setup(self, stage=None):
        if self.val_dir:
            self.train = ImageFolder(self.train_dir, loader=pil_loader_rgba)
            self.val = ImageFolder(self.val_dir, loader=pil_loader_rgba)
        else:
            # self.train, self.val = random_split(ImageFolder(self.train_dir), [0.8, 0.2])
            self.train = ImageFolder(self.train_dir, loader=pil_loader_rgba)
            # NOTE: use TEST SET for validation
            self.val = ImageFolder(self.test_dir, loader=pil_loader_rgba)

        self.test = ImageFolder(self.test_dir, loader=pil_loader_rgba)
