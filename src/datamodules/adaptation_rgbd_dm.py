...
from datamodules.datasets import ImageFolder
...
class AdaptationRGBD(pl.LightningDataModule):
    def __init__(
        self,
        train_src_dirs: str = [],
        train_target_dirs: str = [],
        val_dirs: str = [],
        test_dirs: str = [],
        batch_size: int = 64,
        num_workers: int = 4,
        ...
        image_mean: list = [0.5, 0.5, 0.5, 1.0],
        image_std: list = [0.5, 0.5, 0.5, 1.0],
        resize: bool = False,
    ...
    def setup(self, stage=None):
        self.train_src = [ImageFolder(train_src_dir, loader=pil_loader_rgba) for train_src_dir in self.train_src_dirs]
        self.train_target = [
            ImageFolder(train_target_dir, loader=pil_loader_rgba) for train_target_dir in self.train_target_dirs
        ]
        self.val = [ImageFolder(val_dir, loader=pil_loader_rgba) for val_dir in self.val_dirs]
        self.test = [ImageFolder(test_dir, loader=pil_loader_rgba) for test_dir in self.test_dirs]
